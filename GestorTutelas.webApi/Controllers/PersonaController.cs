...
namespace GestorTutelas.webApi.Controllers
{
    ...
    public class PersonaController : ControllerBase
    {
        ...
        public ActionResult GetAll()
        {
            try
            {
                var entityList = this._PersonaRepository.GetAll();
        ...
        public ActionResult Get(Guid id)
        {
            try
            {
                var entity = this._PersonaRepository.Get(id);
        ...
        [HttpPost]
        public ActionResult Post(PersonaEntity entity)
        {
            try
            {
                entity.Id = Guid.NewGuid();
                var response = this._PersonaRepository.Insert(entity);
                if (response)
                {
                    return Ok(new { status = true, message = entity });
                }
        ...
        [HttpPut]
        public ActionResult Put(PersonaEntity entity)
        {
            try
            {
                var response = this._PersonaRepository.Update(entity);
                if (response)
                {
                    return Ok(new { status = true, message = entity });
                }
        ...
        public ActionResult Delete(Guid id)
        {
            try
            {
                var response = this._PersonaRepository.Delete(id);
